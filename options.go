...
// LogResult holds the output of a container log query.
type LogResult struct {
	StdOut string
	StdErr string
}

// Combined returns stdout and stderr combined into a single string.
func (r LogResult) Combined() string {
	return r.StdOut + r.StdErr
}

// LogOption configures a log query.
...
// WithTail limits output to the last n lines.
// Pass "all" or "" for no limit (default).
func WithTail(n string) LogOption {
	return func(c *logConfig) { c.tail = n }
}

// WithLogsSince filters logs to entries after the given timestamp.
// Accepts RFC3339 timestamps or Unix timestamps as strings.
func WithLogsSince(since string) LogOption {
	return func(c *logConfig) { c.since = since }
}

// WithLogsUntil filters logs to entries before the given timestamp.
// Accepts RFC3339 timestamps or Unix timestamps as strings.
func WithLogsUntil(until string) LogOption {
	return func(c *logConfig) { c.until = until }
}

// WithTimestamps includes a timestamp prefix on each log line.
func WithTimestamps() LogOption {
	return func(c *logConfig) { c.timestamps = true }
}
